...
if(randomNum == 0){
	...
	allimages = [
		{
			'name': 'pictures',
			'images':['puzzle2.png'], //hypercharge
			'dir': ''
		},
	];
}
...
(function( window, undefined ) {
	var js = {
		...
		idealh: 1,
		canvasmode: 1,
		piececountx: 3, //number of pieces across - changed from original 6
		...
		general: {
			...
			releasePiece: function(){
			    document.getElementById("place_music").play();
				if(js.clickedpiece !== -1){
					for(var p = 0; p < js.pieces.length; p++){
					...
					if(js.pieces.length === 0){
					    document.getElementById("end_music").play();
					    document.getElementById("bg_music").pause();
						document.getElementById('body').className = 'solved';
						localStorage.setItem("gameState","won");
						setTimeout(function(){document.querySelector('.modal-backdrop').classList.remove('d-none')},1000);
					}
				}
			},
